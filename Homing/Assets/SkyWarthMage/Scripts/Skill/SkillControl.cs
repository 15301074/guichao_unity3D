...
public class SkillControl : MonoBehaviour {
	...
	public GameObject Skill_4_Lighting;
	public CapsuleCollider Skill_4_Collider;
	public Transform Skill_4_Position;
	...
	void Skill_4_start(){
		Skill_4_Collider.enabled = true;	//技能4碰撞体生效
		GameObject clone_lighting;
		Quaternion quaternion = new Quaternion ();
		quaternion.eulerAngles = new Vector3 (-90f, 0f, 0f);
		clone_lighting = (GameObject)Instantiate (Skill_4_Lighting, Skill_4_Position.position,quaternion);
		clone_lighting.GetComponent<ParticleSystem> ().Play ();
		Destroy (clone_lighting, 3.5f);
	}

	void Skill_4_end(){
		Invoke("Skill4_colleder_end", 3);	//2秒后执行一次Skill4_colleder_end函数
	}

	void Skill4_colleder_end(){
		Skill_4_Collider.enabled = false;
	}
}
