...
public class Item : MonoBehaviour {
	...
	public void Additem(string name, Item.type type, int num) {
		...
		/* 实例化物品按钮 */
		Button itemObj = Instantiate(GameObject.FindGameObjectWithTag("ItemDemo").GetComponent<Button>());
        ...
        if (itemType.Equals (type.consumable) || itemType.Equals (type.activeProp)) {
			...
			if (itemType.Equals (type.consumable)) {	// 消耗品设置数量可见
                ...
                Text numText = itemObj.transform.GetChild(0).GetComponent<Text>();

                itemObj.onClick.AddListener(delegate () {
                    int currentNum = int.Parse(numText.text) - 1;
                    if (currentNum <= 0)
                    {
                        itemObj.gameObject.SetActive(false);
                        GameObject desc = GameObject.FindGameObjectWithTag("Description");
                        if (desc != null)
	...
	/* 给物品按钮修改属性并添加主动效果 */
	private void addItemEvent(Button itemObj) {
		switch (itemName) {
		case "芒果":			// 点击回魔
		...
		case "狗头人的小铁锹":
			...
			itemObj.onClick.AddListener (delegate() {
                ...
                if (Mathf.Abs(x - 188) < 5 && Mathf.Abs(z - 210) < 5) {	// 第一个挖宝点
                                                                                ...
                                                                                // mxl
                    zhujiao = GameObject.FindGameObjectWithTag("Player");
                    zhujiao.transform.position = new Vector3(214, 16.5f, 758f);
