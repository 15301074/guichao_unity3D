...
public class BossAI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        intState = glb.getMonsterFlag(tags);
    ...
    void Update()
    {
        ...
        switch (currentState)
        {
            ...
            case MonsterState.ATTACK:
                ...
                if (Time.time - this.lastActTime >= actRestTme)
                    attackCheck();//改变攻击状态
                break;

            case MonsterState.DIE://死亡
                this.PlayAnimator("creatureDie");
    ...
    private void StandCheck()
    {
        ...
        {
            if (glb.getMonsterFlag(tags) == 0) glb.changeMonsterFlag(tags, 1);
    ...
    void RunCheck()
    {
        ...
        {
            if (glb.getMonsterFlag(tags) == 1) glb.changeMonsterFlag(tags, 0);
    ...
    void SKILLCheck()
    {
        if (Time.time - lastActTime >= this.actRestTme)
        {
            float numbers = Random.Range(0, 1000);
            //if (number <= actionWeight[0])
            if (numbers > 750 && numbers < 1000)
            {
                lastActTime = Time.time;
                currentState = MonsterState.SKILL;
                print("治疗成功");
            }
            else if (numbers < 750)
            {
