...
public class PlayerController : MonoBehaviour {
	...
	void Start () {
		...
		/* 主动道具 */
		Item item = new Item ();
		item.Additem ("藏宝图", "藏宝图的描述", Item.type.activeProp,0);
		item.getItem ().onClick.AddListener (delegate() {
			Debug.Log ("这是一张藏宝图");
		});

		/* 消耗品 */
		item.Additem ("芒果", "芒果的描述", Item.type.consumable,3);
		item.getItem ().onClick.AddListener (delegate() {
			Text num = item.getItem ().transform.GetChild (0).GetComponent<Text> ();
			num.text = (int.Parse(num.text) - 1).ToString();
			Debug.Log ("这是一个芒果");
		});

		/* 装备 */
		item.Additem ("秘法鞋", "秘法鞋的描述", Item.type.pasEquip,0);
		item.getItem ().onClick.AddListener (delegate() {
			Debug.Log ("这是一双秘法鞋");
		});
	}
